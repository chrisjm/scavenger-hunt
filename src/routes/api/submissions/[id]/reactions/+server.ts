...
import { addReaction, removeReaction, REACTION_EMOJIS } from '$lib/server/reactions/reactionService';

const MAX_REACTOR_LIST = 25;

const { submissions, userProfiles, photos, tasks, submissionReactions } = schema;

async function loadSubmissionWithAccess(submissionId: string, userId: string | null, isAdmin: boolean) {
...
export const POST: RequestHandler = async ({ params, request, locals }) => {
  try {
    ...
    await addReaction(
      { submissionId, userId: authUser.userId!, emoji },
      { db }
    );
