...
export const GET: RequestHandler = async ({ locals, url }) => {
	...
	baseQuery
		.orderBy(desc(submissionReactionEvents.createdAt), desc(submissionReactionEvents.id))
		.limit(perPage + 1);
	...
	if (cursor) {
		...
		baseQuery.where(
			or(
				...
				and(
					eq(submissionReactionEvents.createdAt, createdAt),
					lt(submissionReactionEvents.id, eventId)
				)
			)
		);
	}
	...
	if (hasMore) {
		...
		const createdAt =
			last.createdAt instanceof Date ? last.createdAt.getTime() : Number(last.createdAt);
